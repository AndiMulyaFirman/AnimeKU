...
class ListAnimeAdapter(private val listAnimes: ArrayList<Anime>) : RecyclerView.Adapter<ListAnimeAdapter.ListViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        val (name, description, photo) = listAnimes[position]
        ...
        holder.itemView.setOnClickListener {
            onItemClickCallback.onItemClicked(listAnimes[holder.adapterPosition])
        }
        holder.itemView.setOnClickListener {
            val intentDetail = Intent(holder.itemView.context, DetailActivity::class.java)
            intentDetail.putExtra("key_hero", listAnimes[holder.adapterPosition])
